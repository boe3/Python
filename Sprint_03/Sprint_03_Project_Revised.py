...
# Check for fully duplicate rows (Line 24)
# print(orders.duplicated(subset=['user_id', 'order_id', 'order_number', 'order_dow', 'order_hour_of_day', 'days_since_prior_order']).sum())
print(products.duplicated(subset=['product_id', 'product_name',  'aisle_id',  'department_id']).sum())
...
products.drop_duplicates('product_name', inplace=True, ignore_index=True)
